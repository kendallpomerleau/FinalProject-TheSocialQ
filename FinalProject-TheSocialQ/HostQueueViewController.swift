...
class HostQueueViewController: UIViewController, UITableViewDataSource {
    ...
    func cacheImages() {
        ...
        var url = URL(string: currentQueue.playlistSongs[0].coverPath!)
        if !(currentQueue.currentSongPoint+1 >= currentQueue.playlistSongs.count) {
            url = URL(string: currentQueue.playlistSongs[currentQueue.currentSongPoint+1].coverPath!) 
        }
        let data = try? Data(contentsOf: url!)
        if (data != nil){
            let image = UIImage(data:data!)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (indexPath.row >= currentQueue.songs.count-1){
            if currentQueue.currentSongPoint+1 >= currentQueue.playlistSongs.count {
                //ADDED BY KEVIN 11/28
                cellTitle.text = currentQueue.playlistSongs[0].name
                cellDescription.text = currentQueue.playlistSongs[0].artist
                cellImg.image = imageCache[indexPath.row]
            } else {
